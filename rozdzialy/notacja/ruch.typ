...
Bandę oznacza się jako `bar`, zaś dwór jako `off`. Osobiście dopuszczam też~stosowanie łącznika-minusa (ewentuälnie dywizu lub~podobnego znaku). Kontekst zawsze wskazuje, z~czym mamy do~czynienia – `bar` zawsze występuje przed łamaną kreską, zaś `off` zawsze występuje po~łamanej kresce.

`0` (lub `fan`) oznacza niemożliwość wprowadzenia piona.

Numeracja pól jest stosowana dla~bieżącego gracza (aktuälny gracz porusza się w~kierunku malejących numerów pól).
